...
#     #### STEP 3
    basePoint,imgHist = utils.getHistogram(imgWarp,display=True,minPer=0.5,region=4)
    curveAveragePoint, imgHist = utils.getHistogram(imgWarp, display=True, minPer=0.9)
    curveRaw = curveAveragePoint - basePoint
    print(curveRaw)
    # cv2.imshow('maskedddd', imgThres)
    # cv2.imshow('Warpeddd', imgWarp)
    # cv2.imshow('Transformed', imgWarpPoints)
    # print(basePoint)
    # print(imgHist)
...
if __name__ == '__main__':
    cap = cv2.VideoCapture('Pld.mp4')
    intialTrackBarVals = [58, 50, 30, 140 ]
